...
do
	...
	local function getClosestShield()
		...
		for _, tycoon in ipairs(tycoons:GetChildren()) do
			if tycoon.Name == currentTycoon then
				continue
			end

			local owner = tycoon:FindFirstChild("Owner")
			if owner and owner.Value ~= lp.Name then
				local shield = tycoon:FindFirstChild("PurchasedObjects")
					and tycoon.PurchasedObjects:FindFirstChild("Base Shield")
				if shield then
					local shieldPart = shield:FindFirstChild("Shield")
						and shield.Shield:FindFirstChildWhichIsA("BasePart")
					if shieldPart then
...
do
	...
	local function attackShield()
		...
		if not settings then
			settings = { FireRate = 1000, CooldownTime = 4, BulletSpread = 0.8, OverheatCount = 150 }
		end
		local smokePart = cramData.smokePart
		if not smokePart or not smokePart.Parent then
			return
		end
		...
		if normal.Magnitude < 0.000000000000000000000000001 then
			normal = Vector3.new(0, 1, 0)
		end
		local registerTurretHit = game:GetService("ReplicatedStorage"):FindFirstChild("BulletFireSystem")
			and game:GetService("ReplicatedStorage").BulletFireSystem:FindFirstChild("RegisterTurretHit")
		if registerTurretHit then
			pcall(function()
				registerTurretHit:FireServer(cramData.cramModel, smokePart, cramData.cram, {
					normal = normal,
					hitPart = shield,
					origin = origin,
					hitPoint = targetPos,
	...
	local function attackTarget(cramData, target, settings)
		if not cramData or not target then
			return
		end
		local smokePart = cramData.smokePart
		if not smokePart or not smokePart.Parent then
...
do
	...
	local function getVehicleTargets()
		...
		for _, player in ipairs(game:GetService("Players"):GetPlayers()) do
			...
			if table.find(vehicleKillWhitelist, player.Name) then
				continue
			end
...
do
	local littlebirdKillEnabled = false
	local littlebirdKillLastAttack = 0
	local littlebirdKillInterval = 0.1
	local littlebirdKillLoop = nil
	local littlebirdKillWhitelist = {}
	local attackShieldsEnabledLB = false
	local shieldOnlyLoop = nil

	local littlebirdWhitelistDropdown = KillTab:Dropdown({
		Title = "小鸟杀戮白名单",
		Values = {},
		Multi = true,
		AllowNone = true,
		Callback = function(selected)
			littlebirdKillWhitelist = {}
			for _, v in pairs(selected) do
				table.insert(littlebirdKillWhitelist, WhitelistManager:GetPlayerName(v))
			end
		end,
	})

	WhitelistManager:RegisterRefreshCallback("LittlebirdKill", function(list)
		littlebirdWhitelistDropdown:Refresh(list)
	end)

	local function getLittlebirdTargets()
		local lp = game:GetService("Players").LocalPlayer
		local targets = {}
		...
		for _, player in ipairs(game:GetService("Players"):GetPlayers()) do
			...
			if table.find(littlebirdKillWhitelist, player.Name) then
				continue
			end
	...
	local function getClosestShieldLB()
		local lp = game:GetService("Players").LocalPlayer
		if not lp.Character or not lp.Character:FindFirstChild("HumanoidRootPart") then
			return nil
		end
		...
		for _, tycoon in ipairs(tycoons:GetChildren()) do
			...
			if owner and owner.Value ~= lp.Name then
				local shield = tycoon:FindFirstChild("PurchasedObjects")
					and tycoon.PurchasedObjects:FindFirstChild("Base Shield")
				if shield then
					local shieldPart = shield:FindFirstChild("Shield") and shield.Shield:FindFirstChild("Shield1")
	...
	local function getRocketLaunchers()
		local vehicleWorkspace = workspace:FindFirstChild("Game Systems")
		...
		local vehicle = vehicleWorkspace:FindFirstChild("AH-6 Littlebird")
		if not vehicle then
			return nil
		end
		...
		return turret:FindFirstChild("Rocket Launchers")
	end

	local function fireLittlebirdRocket(target)
		local rocketLaunchers = getRocketLaunchers()
		if not rocketLaunchers then
			return
		end
		...
		local targetPos = target.position
		local originPos = targetPos + Vector3.new(0, 50, 0)
		local direction = (targetPos - originPos).Unit
		...
		local args = {
			{
				...
				HitPart = target.hrp,
				Origin = originPos,
				Label = target.player.Name .. "Rocket" .. math.random(1000, 9999),
				Vehicle = vehicle,
				Position = targetPos,
				Weapon = rocketLaunchers,
			},
		}
		...
		local events = rocketSystem:FindFirstChild("Events")
		if not events then
			return
		end

		local rocketHit = events:FindFirstChild("RocketHit")
		if not rocketHit then
			return
		end

		pcall(function()
			rocketHit:FireServer(unpack(args))
		end)
	end

	local function fireLittlebirdRocketAtShield(shieldPart)
		local rocketLaunchers = getRocketLaunchers()
		if not rocketLaunchers then
			return
		end
		...
		local args = {
			{
				...
				HitPart = shieldPart,
				Origin = originPos,
				Label = "ShieldRocket" .. math.random(1000, 9999),
				Vehicle = vehicle,
				Position = targetPos,
				Weapon = rocketLaunchers,
			},
	...
	local function littlebirdKillMainLoop()
		while littlebirdKillEnabled do
			local now = tick()

			if now - littlebirdKillLastAttack >= littlebirdKillInterval then
				local targets = getLittlebirdTargets()
				for _, target in ipairs(targets) do
					task.spawn(function()
						fireLittlebirdRocket(target)
					end)
				end

				littlebirdKillLastAttack = now
			end
	...
	local function shieldOnlyMainLoop()
		while attackShieldsEnabledLB do
			local shield = getClosestShieldLB()
			if shield then
				for i = 1, 10 do
					task.spawn(function()
						fireLittlebirdRocketAtShield(shield)
					end)
				end
			end
			task.wait(0.1)
	...
	KillTab:Toggle({
		...
		Value = false,
		Callback = function(state)
			littlebirdKillEnabled = state

			if state then
				if littlebirdKillLoop then
					task.cancel(littlebirdKillLoop)
					littlebirdKillLoop = nil
				end
				littlebirdKillLoop = task.spawn(littlebirdKillMainLoop)
			else
				if littlebirdKillLoop then
					task.cancel(littlebirdKillLoop)
					littlebirdKillLoop = nil
	...
	KillTab:Toggle({
		...
		Value = false,
		Callback = function(state)
			attackShieldsEnabledLB = state

			if state then
				if shieldOnlyLoop then
					task.cancel(shieldOnlyLoop)
					shieldOnlyLoop = nil
				end
				shieldOnlyLoop = task.spawn(shieldOnlyMainLoop)
			else
				if shieldOnlyLoop then
					task.cancel(shieldOnlyLoop)
					shieldOnlyLoop = nil
...
do
	...
	local function fireRocket(targetPlayer)
		...
		local rocketEvent = rs:FindFirstChild("RocketSystem")
			and rs.RocketSystem:FindFirstChild("Events")
			and rs.RocketSystem.Events:FindFirstChild("RocketHit")
				...
				task.spawn(function()
					while rpgBombingEnabled do
						...
						for _, targetPlayer in ipairs(players) do
							...
							if table.find(rpgBombingWhitelist, targetPlayer.Name) then
								continue
							end
				...
				task.spawn(function()
					while shieldAttackActive and not rpgBombingEnabled do
						...
						if shield then
							...
							local character = lp.Character
							local hrp = character and character:FindFirstChild("HumanoidRootPart")
							local rpg = lp.Backpack:FindFirstChild("RPG")
								or (character and character:FindFirstChild("RPG"))
