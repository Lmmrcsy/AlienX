...
local A = FunSection:Tab({Title = "调试功能", Icon = "settings"})
    ...
    Callback = function(t)
        ...
        if t then
            ...
            rpgAttackThread = coroutine.create(function()
                ...
                -- 动态攻击模式
                local attackModes = {
                    {name = "快速突击", duration = 2.0, pause = 1.5, intensity = 0.9},
                    {name = "标准攻击", duration = 3.5, pause = 2.0, intensity = 0.7},
                    {name = "精准打击", duration = 2.8, pause = 2.5, intensity = 0.6},
                    {name = "游击模式", duration = 1.8, pause = 3.0, intensity = 0.8}
                }
                
                local currentMode = math.random(#attackModes)
                ...
                while loopActive do
                    ...
                    local mode = attackModes[currentMode]
                    
                    -- 智能模式切换
                    if attackCount >= 8 or totalAttackTime > 30 then
                        currentMode = math.random(#attackModes)
                        attackCount = 0
                        totalAttackTime = 0
                    ...
                    if attackPhase == "attack" then
                        if elapsed >= mode.duration then
                            ...
                            if character and character:FindFirstChild("HumanoidRootPart") then
                                ...
                                local offsetY = math.random(850, 1250)
                                local offsetZ = math.random(-60, 60)
                                
                                local attackPosition = character.HumanoidRootPart.Position + 
                                ...
                                if weapon then
                                    ...
                                    local maxTargets = math.random(1, math.min(3, #validPlayers))
                                    
                                    for i = 1, maxTargets do
                                        if not loopActive or i > #validPlayers then break end
                                        ...
                                        if player.Character then
                                            ...
                                            if target then
                                                -- 模拟人类瞄准误差
                                                local shouldMiss = math.random(100) <= 8
                                                local actualPosition = attackPosition
                                                
                                                if shouldMiss then
                                                    actualPosition = actualPosition + Vector3.new(
                                                        math.random(-150, 150),
                                                        math.random(-100, 100),
                                                        math.random(-150, 150)
                                                    )
                                                ...
                                                -- 执行攻击
                                                local success = pcall(function()
                                                    FireRocket:InvokeServer(Vector3.new(), weapon, weapon, actualPosition)
                                                ...
                                                if success then
                                                    ...
                                                    totalAttackTime = totalAttackTime + elapsed
                                                end
                                                
                                                -- 智能攻击间隔
                                                networkAwareWait(0.12 * mode.intensity, 0.25 * mode.intensity)
                                            end
                                        end
                                    end
                                end
            ...
            shieldAttackThread = coroutine.create(function()
                ...
                while shieldAttackActive do
                    local success, err = pcall(function()
                        ...
                        -- 动态攻击位置
                        local attackPos = HumanoidRootPart.Position + Vector3.new(
                            math.random(-40, 40),
                            math.random(950, 1150),
                            math.random(-40, 40)
                        )
                        ...
                        -- 随机排序目标
                        for i = #tycoons, 1, -1 do
                            local j = math.random(i)
                            tycoons[i], tycoons[j] = tycoons[j], tycoons[i]
                        end

                        local attackedCount = 0
                        local maxAttacksPerSession = math.random(2, 4)
                        
                        for _, tycoon in ipairs(tycoons) do
                            if not shieldAttackActive or attackedCount >= maxAttacksPerSession then break end
                                ...
                                if shield then
                                    ...
                                    for i = 1, shotCount do
                                        ...
                                        if attackSuccess then
                                            ...
                                            attackSessionCount = attackSessionCount + 1
                                        end
                                        
                                        -- 递增攻击间隔
                                        networkAwareWait(0.15 + (i * 0.06), 0.25 + (i * 0.1))
                                    end
                        ...
                        -- 会话冷却
                        if attackSessionCount >= 12 then
                            networkAwareWait(3.0, 6.0)
                            attackSessionCount = 0
                        end
                    end)
...
coroutine.wrap(function()
    while true do
        ...
        -- 如果检测到异常模式，自动暂停
        if loopActive and os.clock() - lastAttackTime > 10 then
            loopActive = false
            warn("安全系统: 检测到异常，自动停止RPG攻击")
        end
        
        if shieldAttackActive and os.clock() - lastShieldAttack > 15 then
            shieldAttackActive = false
            warn("安全系统: 检测到异常，自动停止护盾攻击")
        end
    end
end)()
...
WeaponTab:Button({
    ...
    Callback = function()
        ...
        for _, tycoon in pairs(workspace.Tycoon.Tycoons:GetChildren()) do
            local purchasedObjects = tycoon:FindFirstChild("PurchasedObjects")
            if purchasedObjects then
                local giver = purchasedObjects:FindFirstChild("Remington MSR Giver")
                if giver then
...
CarTab:Button({
    ...
    Callback = function()
        local Workspace = game:GetService("Workspace")
        local CONFIG = {
            FIRE_RATE = {DEFAULT = 9999, DOUGLAS = 300, MOUNTED = 70, HOVERCRAFT = 9999},
        ...
        local function disableVehicleSeats(vehicle)
        ...
        local function modifyWeaponSettings(settings, workspaceName, weaponName)
            if not settings then return end
            if workspaceName == "Tank Workspace" then
                if weaponName:lower():find("mounted") or weaponName:lower():find("cannon") then
                    settings.FireRate = CONFIG.FIRE_RATE.MOUNTED
                end
            else
                settings.FireRate = CONFIG.FIRE_RATE.HOVERCRAFT
            end
            if settings.OverheatCount ~= nil then settings.OverheatCount = CONFIG.OVERHEAT_COUNT end
            if settings.CooldownTime ~= nil then settings.CooldownTime = CONFIG.COOLDOWN_TIME end
            if settings.DepleteDelay ~= nil then settings.DepleteDelay = CONFIG.DEPLETE_DELAY end
            if settings.OverheatIncrement ~= nil then settings.OverheatIncrement = CONFIG.OVERHEAT_INCREMENT end
            if settings.magnitude ~= nil then settings.magnitude = CONFIG.RECOIL.MAGNITUDE end
            if settings.roughness ~= nil then settings.roughness = CONFIG.RECOIL.ROUGHNESS end
            if settings.posInfluence ~= nil then settings.posInfluence = CONFIG.RECOIL.POS_INFLUENCE end
            if settings.rotInfluence ~= nil then settings.rotInfluence = CONFIG.RECOIL.ROT_INFLUENCE end
        end

        local function processWorkspace(workspaceName, processor)
            local workspaceFolder = Workspace:FindFirstChild("Game Systems") and Workspace["Game Systems"]:FindFirstChild(workspaceName)
            if workspaceFolder then
                for _, vehicle in ipairs(workspaceFolder:GetChildren()) do
                    pcall(processor, vehicle, workspaceName)
                end
            end
        end
        ...
        local function processVehicle(vehicle, workspaceName)
            ...
            for _, weaponSystem in ipairs(turrets:GetChildren()) do
                for _, weapon in ipairs(weaponSystem:GetChildren()) do
                    if (weapon:IsA("Folder") or weapon:IsA("Model")) and weapon:FindFirstChild("Settings") then
                        removeParticles(weapon)
                        local settings = safeRequire(weapon.Settings)
                        modifyWeaponSettings(settings, workspaceName, weapon.Name)
                    end
                end
            end
        end

        processWorkspace("Tank Workspace", processVehicle)
        processWorkspace("Vehicle Workspace", processVehicle)
        processWorkspace("Helicopter Workspace", processVehicle)
        processWorkspace("Plane Workspace", processVehicle)
        processWorkspace("Boat Workspace", processVehicle)

        processWorkspace("Hovercraft Workspace", function(hovercraft, workspaceName)
            removeParticles(hovercraft)
            disableVehicleSeats(hovercraft)
            local misc = hovercraft:FindFirstChild("Misc")
            if not misc then return end
            local turrets = misc:FindFirstChild("Turrets")
            if not turrets then return end
            for _, weaponSystem in ipairs(turrets:GetChildren()) do
        ...
        print("载具篡改完成！包括所有气垫船武器修改。")
    end
})



local toggleKeyInput = A:Input({
    Title = "设置快捷键",
    Value = "G",
    Placeholder = "输入快捷键",
    Callback = function(input)
